...
export default function UpdateFieldForm({
  field = {},
  onSave,
}: {
  field?: any;
  ...
  const [options, setOptions] = useState(
    field.options?.map((o, idx) => ({ id: String(idx + 1), value: o })) || []
  );
  ...
  const validate = () => {
    if (
      ...
      const hasEmpty = options.some((opt) => !opt.value.trim());
      setError(hasEmpty);
      return !hasEmpty;
    }
    return true;
  ...
  const handleSubmit = () => {
    ...
    console.log("Saving field:", payload);
    onSave?.(payload);
  };
